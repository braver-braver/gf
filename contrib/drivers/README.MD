...
- Supports `LastInsertId` for numeric primary keys (for example, sequence/trigger-based auto IDs).
- `InsertIgnore` without explicit `OnConflict` falls back to normal insert and ignores duplicate-key conflicts.
